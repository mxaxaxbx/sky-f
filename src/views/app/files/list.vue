<template>
  ...
  >
    ...
    <Modal v-model="moveToFolderModal" size="xl" @click.stop>
      <template #header>
        <h3 class="">Move:
          <p class="font-normal text-sm mt-2 break-all w-[90%]" v-for="file in selectedFiles" :key="file.id">
            {{ file.name }}
          </p>
        ...
        >
          ...
          <div class="flex-1 flex items-center justify-center overflow-hidden h-full w-full rounded-2xl bg-white/5">
            ...
            <div v-else class="flex flex-col pb-10 items-center text-[var(--text-terceary)] text-xs text-center sm:text-md">
              ...
              <span class="font-semibold text-white text-xl mb-5 sm:text-3xl"> {{ previewFile.name }} </span>
